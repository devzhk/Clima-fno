...
# the model is a function: q,t ->  M(q)
# the solution points are at cell faces
def implicit_Newton_solve(model_jac, f, dbc, dt=1.0, Nt=1000, save_every=1, L=1.0, Newton_eps=1e-3,
                          Newton_maxiterstep=1000):
    Ny = f.size
    yy = np.linspace(0, L, Ny)
        ...
        while not Newton_converge:
            ...
            if (np.linalg.norm(res_all) < Newton_eps or Newton_iterstep > Newton_maxiterstep):
                if Newton_iterstep > Newton_maxiterstep:
                    print("Newton iteration does not converge :", Newton_iterstep, " error is : ",
                          np.linalg.norm(res_all))
...
def nummodel_jac(permeability, q, yy, res, V, exact=False, D_permeability=None):
    ...
    for i in range(Ny - 2):
        if i == 0:
            ...
            V[2 + 3 * (i - 1) + 0], V[2 + 3 * (i - 1) + 1] = 1 / dy ** 2 * mu_c[i], -1 / dy ** 2 * (
                        mu_c[i] + mu_c[i + 1])
        else:
            V[2 + 3 * (i - 1) + 0], V[2 + 3 * (i - 1) + 1], V[2 + 3 * (i - 1) + 2] = 1 / dy ** 2 * mu_c[
                i], -1 / dy ** 2 * (mu_c[i] + mu_c[i + 1]), 1 / dy ** 2 * mu_c[i + 1]
    if exact:
        Dq_c, Ddq_c = D_permeability(q_c, dq_c)

        for i in range(Ny - 2):
            # diagonal i,i : q_{i+1}
            V[3 * i] += (q[i + 2] - q[i + 1]) / dy ** 2 * (1 / 2 * Dq_c[i + 1] - Ddq_c[i + 1] / dy) - (
                        q[i + 1] - q[i]) / dy ** 2 * (1 / 2 * Dq_c[i] + Ddq_c[i] / dy)
        for i in range(Ny - 3):
            # +1 diagonal
...
def generate_data(f_funcs, Nx=100):
    ...
    xx, f, q, q_c, dq_c = np.zeros((n_data, Nx)), np.zeros((n_data, Nx)), np.zeros((n_data, Nx)), np.zeros(
        (n_data, Nx - 1)), np.zeros((n_data, Nx - 1))
